    ...
    public function precontroller($filter){
        if( $this->_applies($this->filter[$filter]) ){            
            if( $this->uri->segment(1)!="panel" ) {
                $this->ci->assets->add_js_default(array('plugins/simplemodal/js/jquery.simplemodal', 'general'));
                $this->ci->assets->add_css_default(array('style_panel'));
                if( $this->ci->agent->browser()=="Internet Explorer" ){
                    if( $this->ci->agent->version()>=6 ){
                        $this->ci->assets->add_css_default(array('plugins_simplemodalie6'));
                    }elseif( $this->ci->agent->version()==7 ){
                        $this->ci->assets->add_css_default(array('plugins_simplemodalie7'));
                    }
                }else{
                    $this->ci->assets->add_css_default(array('plugins_simplemodal'));
                }
            }else{
                $this->ci->assets->add_css_default(array('style_panel'));
